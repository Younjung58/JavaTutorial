...
public class _test_04 {
	...
	public static void main(String[] args) {
		...
		// 행은 0,1,2 ..의 순이고, 열은 그의 역순이다.
		// 다만, 열의 최대값인 4에 도달했을때부터는
		// 열의 시작이 열의 최대값으로 고정되어지고
		// 그에따라 행의 값이 행의 최대값까지로 진행되도록 바뀐다.(if문으로 구분 필요)
		...
		for (int i = 0; i < leng; i++) {
			if(i<a[0].length-1) {			// 열의 최대값에 도달 전
				for (int j = i; j >= 0; j--) {
					a[i-j][j]=num;
					num++;
				}
			}else {
				for (int j = a[0].length-1; i-j < a.length; j--) {	// 열 최대값 도달 이후
					a[i-j][j]=num;
					num++;
				}
			}
